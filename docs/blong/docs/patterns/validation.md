...
Validations can be automatically derived from types defined for the handler parameters and result,
...
- Put a file named `~.schema.ts` in the folder where the handlers are defined. This file is
  automatically updated when its date is older than another file within the folder, where a type
  named `Handler` is defined.
- Define the type `Handler` for each handler that will use automatic validations. Here is an example
  how it must be defined:
...
To define validations manually, put them in a subfolder of the `gateway` layer and use the
`validation` function from the framework. The framework will provide access to the
[TypeBox](https://github.com/sinclairzx81/typebox) API via the `lib.type` property.
