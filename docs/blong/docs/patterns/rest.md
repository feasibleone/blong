...
- use the `api` function to define namespaces for each API
    ...
    export default api(() => ({
        namespace: {
            clock: [
                'test/framework/api/world-time.yaml',
                'test/framework/api/world-time.operations.yaml',
            ],
        },
    }));
    ```

    This will merge the OpenAPI definitions in the given files into a single namespace `clock` and
    mount them at `/rest/` path of the gateway. Merging is only needed if the original definition is
    not good enough, for example it does not include the `operationId` field or it is not using the
    desired naming convention.

- Implement the handlers, by prefixing the operationId with the namespace. Only paths with HTTP
  methods that have an `operationId` can have handlers.
