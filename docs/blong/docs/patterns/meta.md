...
To implement various kinds of functionality the framework uses a metadata object, which is passed to
the handlers. The convention is to use a variable named `$meta` to refer to that parameter.

The metadata may contain the following properties:

- `mtid`: the type of message being processed, one of `request`, `response`, `notification`,
  `error`, `event`
...
- `dispatch`: optional function to be called during the `dispatch` step of the
  [adapter loop](../concepts/adapter#adapter-loop), instead of the framework's built-in dispatch
  function
...
These properties are used for the audit log and are set by the [gateway](../concepts/gateway), based
on information from the HTTP server and the OS:
