...
The pages described here are generated by `demo/blong-marine`, whose Storybook shows them against
fixture data (`cd demo/blong-marine && npm run storybook`, port 6007). The conventions for running
one are in the [Storybook pattern](../patterns/blong-browser.md#storybook-pattern).
...
```mermaid
flowchart TD
    A["IModelSpec.schema overlay<br/>(static, in code)"] --> B["{subject}.{object}.schema handler<br/>(runtime, from backend — returns {} if none)"]
    B --> C["blong.lib.merge(model.schema, schemaOverride)"]
    C --> D["IEnrichedSchema<br/>(passed to Editor / Report)"]
    D --> E1["Editor browse layout<br/>table + navigator + detail"]
    D --> E2["Editor form cards + widgets"]
    D --> E3["Report filter + table"]
```
