...
configuration. This is intended to run only during deployment — typically as a dedicated short-lived
job — **not** on every normal application startup:
...
2. **Auto-binds CRUD handlers** for every declared table when a `namespace` is configured. The
   generated handlers (`${namespace}${Table}Get`, `Find`, `Add`, `Edit`, `Remove`, `Merge`) are
   reachable via normal dispatch without any handler files.
