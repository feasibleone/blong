...
- **Causal lineage and intent travel with the request.** A **trace** id follows one request across
  ...
  it. Business intent is attached once, at the entry point, and inherited below it.
...
local cache with no service anywhere, and ships the same records to one when configured.
