 ...
 *   CI_REPORTS_BASE — Pages URL of the reports repository; when set (and a
 *     workflow + run number are known) the report links every published report,
 *     which is what lets the action summary and the PR comment be one document
...
export async function ciReport(args: string[]): Promise<void> {
    ...
    const runAllure = (allureArgs: string[], allureCwd: string) =>
        runTool('allure', allureArgs, {
            cwd: allureCwd,
            env: toolEnv(allureCwd),
        } satisfies RunOptions);

    const baseMetricsFile =
        readFlag(args, '--baseline') ??
        process.env['CI_BASE_METRICS'] ??
        join(outDir, 'baseline.json');
    const baseHistoryFile =
        readFlag(args, '--base-history') ?? process.env['CI_BASE_HISTORY'] ?? '';
    ...
    // resolves it), the workflow slug and run number from the environment.
    const reportsBase = (process.env['CI_REPORTS_BASE'] ?? '').replace(/\/+$/, '');
    const workflow = (process.env['GITHUB_WORKFLOW'] ?? '').replace(/\s+/g, '-');
    const links: IReportLinks | null =
        reportsBase && workflow && run
            ? {
                  base: reportsBase,
                  workflow,
                  run,
                  packages: collectPublishable(root, reports),
                  coverage: existsSync(join(root, 'coverage', 'lcov-report')),
                  failures: bundle !== null,
              }
            : null;
    ...
    if (baseline || baseHistoryFile) {
        ...
        const slices = collectHistorySlices(
            root,
    ...
    process.stdout.write(
        `# report: report-data/ci-summary.json, coverage: ${coverage ? 'yes' : 'no'}\n`,
    );
}
