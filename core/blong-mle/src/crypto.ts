/**
 * MLE client crypto — extracted from `blong-gogo/src/jose.ts` so it can be
 * reused by the codec and by dev tooling (e.g. the `blong-dev proxy` command).
 *
 * Implements the Message Layer Encryption (MLE) scheme used by the Blong
 * gateway: every request payload is signed with the client's private sign key
 * and encrypted to the server's public encrypt key (with the client's public
 * sign/encrypt keys carried in the JWE protected header); every response is
 * encrypted to the client's public encrypt key and signed by the server.
 */
import {
    CompactEncrypt,
    CompactSign,
...
/**
 * Create the MLE crypto primitives for a client (or server) with the given
 * sign/encrypt key specs.  `keys` exposes the PUBLIC JWKs (for handshakes);
 * `signEncrypt`/`decryptVerify` are bound to this instance's private keys.
 */
export async function createMleCrypto({
    sign,
    encrypt,
}: {
    sign?: KeySpec;
