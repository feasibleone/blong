/**
 * PrimeReact locale data bundled with blong-browser.
 *
 * Registered via `IThemeConfig.languages` on `<App>` (the default theme
 * includes them), so `appStore.setLanguage('bg')` activates a working
 * PrimeReact locale without per-app configuration.  Suites can override or
 * extend via their own `theme.languages`.
 */
// cspell:disable — translation strings are Bulgarian by design.
