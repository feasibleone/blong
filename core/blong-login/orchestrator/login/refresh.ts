...
export default library<{keys: {refresh: string; refreshCbc: ReturnType<typeof cbc>}}>(
    ({
        config: {
            keys: {refresh, refreshCbc = cbc(refresh)},
        },
        handler: {errorLoginRefreshTokenExpired},
    }) => ({
        writeRefresh(params: {refresh: number}) {
            return refreshCbc
                .encrypt(JSON.stringify({...params, expire: Date.now() + params.refresh * 1000}))
                .toString('base64');
        },
