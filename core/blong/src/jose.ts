...
async function importKey(jwk: JWK | KeyLike | Uint8Array): Promise<{ key: KeyLike | Uint8Array; alg: string; }> {
...
async function exportKey(key: KeyLike | Uint8Array | JWK, priv: boolean = false): Promise<JWK> {
    const jwk: JWK = await isKey(key) ? await exportJWK(key as KeyLike | Uint8Array) : key as JWK;
...
async function verify(plaintext: string | Uint8Array, {key}: {key: KeyLike | Uint8Array}): Promise<object> {
...
async function decryptVerify(
    ...
    return verify(
        await decrypt(message, mlek) as Uint8Array,
        mlskPub
    );
}
...
export default async function jose({sign, encrypt}: {sign: JWK, encrypt: JWK}): Promise<{
    ...
    decrypt: (msg: string | GeneralJWE, options: unknown) => unknown
