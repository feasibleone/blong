...
tap.test("Strategy A — autoSnapshot: true (fully automatic)", async t => {
    ...
    const steps = [
        ...
        async function createQuote(_assert: unknown, context: any) {
            const party = await context.resolveParty;
            return {
                quoteId: QUOTE_ID,
        ...
        async function executeTransfer(_assert: unknown, context: any) {
            const quote = await context.createQuote;
            return {
    ...
    await executor.execute(steps as any, {}, t);
});
    ...
    const steps = [
        ...
        async function createQuote(_assert: unknown, context: any) {
            ...
            return {
                quoteId: QUOTE_ID,
                receiverPartyId: party.partyId as string,
                sendAmount: 100, sendCurrency: "USD",
                receiveAmount: 99.5, receiveCurrency: "USD",
        ...
        async function verifyTransfer(_assert: unknown, context: any) {
            const transfer = await context.executeTransfer;
            return {
...
tap.test("Strategy C — phase checkpoints", async t => {
    ...
    const steps = [
        ...
        async function buildDashboard(_assert: unknown, context: any) {
            const profile = await context.fetchProfile;
            const config  = await context.fetchConfig;
            return {title: (profile.displayName as string) + "'s dashboard", theme: config.theme as string};
        },

        async function renderWidget(_assert: unknown, context: any) {
            const config = await context.fetchConfig;
            return {rendered: true, widgetId: WIDGET_ID, language: config.language as string};
        },
...
tap.test("Strategy D — assert.snapshot() no-args per-step", async t => {
    ...
    const steps = [
        async function resolveParty(assert: any) {
            assert.snapshot();   // executor snapshots return value as "resolveParty"
            return {partyId: PARTY_ID, partyName: "Alice", partyType: "MSISDN", verified: true};
        },

        async function createQuote(assert: any, context: any) {
            const party = await context.resolveParty;
            assert.snapshot();
            return {
                quoteId: QUOTE_ID,
        ...
        async function executeTransfer(assert: any, context: any) {
            const quote = await context.createQuote;
            assert.snapshot();
            return {
                transferId: TRANSFER_ID,
...
tap.test("Hybrid — business assertions + sentinel snapshots + end-of-chain", async t => {
    ...
    const steps = [
        async function provisionAccount(assert: any) {
            const result = {
                accountId: ACCOUNT_ID, balance: 1000,
        ...
        async function addBeneficiary(assert: any, context: any) {
            const account = await context.provisionAccount;
            const result = {
        ...
        async function sendPayment(assert: any, context: any) {
            const beneficiary = await context.addBeneficiary;
            const result = {
        ...
        async function verifyBalance(_assert: unknown, context: any) {
            const payment = await context.sendPayment;
            return {accountId: ACCOUNT_ID, currency: payment.currency as string, newBalance: 950};
        },
