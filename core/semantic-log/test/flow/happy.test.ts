 ...
 *
 * The two topologies share the payer, the receiving provider and the payee; the
 * inter one replaces the scheme-internal hub with the corridor it crosses — the
 * originating scheme's hub A and its **own** provider, the proxy, and the
 * receiving scheme's hub B. Two providers, not one: they are what makes the
 * corridor's rate and the originating scheme's indication different numbers.
 */
const INTER_PARTICIPANTS = ['payer', 'hubA', 'fxpA', 'proxy', 'hubB', 'fxp', 'payee'] as const;
...
t.test('the transfer settles and every participant logs in one trace (PRD R7)', async t => {
    await runFlow(async ({flow, result, records}) => {
        ...
        t.match(trace ?? '', /^tr-\d+$/, 'the entry participant minted it — the driver supplied none');
        ...
        const discovered = records.find(record => record.service === 'payer' && record.msg === 'payee found');
        t.equal(
        ...
        t.equal(flow.participants.length, PARTICIPANTS.length, 'the handle reports the same topology');
    });
});

t.test('one execution is one flow id, carried by every participant, and it is not the trace (D1/D3)', async t => {
    await runFlow(async ({records}) => {
        ...
        const traces = new Set(records.map(record => record.refs.trace));
        t.equal(traces.size, 1, 'one trace, so the two identities can be compared on the same records');
        t.not(flowId, [...traces][0], 'the flow id is a different value from the trace — trace is not the flow');

        const kinds = new Set(records.map(record => record.flow?.kind));
        t.same([...kinds], ['transfer.single'], 'one stable process name: the drift key, not the execution id');
    });
});

t.test('each participant is identifiable and the causal chain is walkable from the records (PRD R7)', async t => {
    await runFlow(async ({records}) => {
        for (const name of PARTICIPANTS) {
            t.ok(records.some(record => record.service === name), `${name} logged`);
        }
        ...
        const dangling = linked.filter(record => !byId.has(record.refs.parent as string)).map(record => record.id);
        ...
        t.match(renderHuman(records.find(record => record.id === tip) as LogRecord), new RegExp(`p=${parent}`), 'the parent link renders in the reference group');
    });
});
...
t.test('references are locally minted and carry a template-id prefix (PRD R19, R12)', async t => {
    await runFlow(async ({records}) => {
        ...
        for (const record of records) {
            t.equal(record.refs.record, record.id, `${record.id}: the record ref is the record's own id`);
            t.ok(isUlid(record.refs.record), `${record.id}: the record ref is a locally minted ULID`);
            t.equal(
        ...
        t.match(refUri('record', records[0].id), /^semantic-log:\/\/record\/[0-9A-Z]{26}$/, 'the ref URI is dereferenceable');
    });
});

t.test('the records alone reconstruct the chain through the service (PRD R7 acceptance)', async t => {
    await runFlow(async ({records}) => {
        ...
        try {
            ...
            t.equal(lineage.rootOf(tip), fromRecords[0], "and it starts at the payer's first record");
            t.same(lineage.traceIds(), [...new Set(records.map(record => record.refs.trace))], 'one trace was indexed');
        } finally {
            await sink?.flush();
            await service.close();
        }
...
t.test('the inter-scheme flow crosses the proxy in one execution and one trace (PRD R9, R11)', async t => {
    await runFlow(
        async ({flow, result, records}) => {
            ...
            t.equal(flow.participants.length, INTER_PARTICIPANTS.length, 'the handle reports the same topology');
            ...
            const flowIds = new Set(records.map(record => record.flow?.id));
            t.equal(flowIds.size, 1, 'one execution still means one flow id across the cross-border hop');
            t.ok(isUlid([...flowIds][0]), 'and it is the caller-minted execution ULID');

            const traces = new Set(records.map(record => record.refs.trace));
            t.equal(traces.size, 1, 'seven participants, one trace');
            t.not([...flowIds][0], [...traces][0], 'the flow id is still a different value from the trace');

            const kinds = new Set(records.map(record => record.flow?.kind));
            t.same([...kinds], ['transfer.inter'], 'the inter flow is its own recurring process — its own drift key');
            ...
            const routed = records.find(record => record.service === 'proxy' && record.msg === 'routing to target ecosystem');
            t.ok(routed, 'the proxy recorded the routing hop it performed');
            t.equal(routed?.decision?.discriminator, 'route-selection', 'the record names the discriminator consulted');
            ...
            // Both schemes price the corridor, and they price it differently. Hub A
            // records the pair and the payer prices the corridor's, so a hub that
            // answered with its own indication instead of the crossed quote would settle
            // on a rate nobody quoted. Nothing else in this file can tell those apart —
            // which is why the two providers are configured to disagree and both values
            // are read back out of the records the run retained rather than supplied.
            const assembled = records.find(
                record => record.service === 'hubA' && record.msg === 'cross-scheme quote assembled',
            );
            t.equal(assembled?.fields?.localRate, 1.12, "the originating scheme's own provider is recorded");
            t.equal(assembled?.fields?.localStatus, 200, 'with its own status');
            t.equal(assembled?.fields?.rate, 1.1, "and the corridor's rate beside it");
            t.not(assembled?.fields?.localRate, assembled?.fields?.rate, 'the two schemes quote differently');

            const priced = records.find(record => record.service === 'payer' && record.msg === 'quote accepted');
            t.equal(priced?.fields?.rate, 1.1, 'and the payer priced the corridor quote, not the local indication');
            ...
            t.equal(withWithheld.length, 0, 'nothing withheld crosses the corridor unless something fails');
        },
        'inter',
    );
});

t.test('an inter-scheme refusal is attributed to the far end and releases the origin hold (PRD R15, R10)', async t => {
    await runFlow(
        async ({result, records}) => {
            ...
            t.equal(traces.size, 1, 'PRD R15: one incident across the corridor, not one alert per scheme');
            ...
            const failed = records.find(record => record.service === 'hubA' && record.msg === 'inter-scheme settlement failed');
            t.ok(failed, 'hub A recorded the failed settlement');
            const withheld = failed?.fields?.withheld as Array<{fields?: Record<string, unknown>}> | undefined;
            t.ok(Array.isArray(withheld) && withheld.length > 0, 'and released the detail it had withheld');
            t.ok(
