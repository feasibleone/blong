...
const flow: IngestEvent[] = [
    ...
    event({id: 'n3', time: 30, service: 'payee', refs: {record: 'n3', trace: 'tr-1', parent: 'n2'}}),
];
...
t.test('a chain is walked from root to leaf (PRD R7 acceptance)', t => {
    ...
    t.same(index.chain('n3').map(node => node.id), ['n1', 'n2', 'n3']);
...
t.test('a trace spans services in time order, parent links or not', t => {
    ...
    t.same(index.trace('tr-1').map(node => node.service), ['payer', 'hub', 'payee', 'payee']);
    t.end();
});
...
t.test('a cycle in malformed parent links cannot hang the walk', t => {
    ...
    t.same(index.chain('a').map(node => node.id), ['b', 'a'], 'the walk returns the cycle once, ending where it re-entered');
    t.same(index.chain('b').map(node => node.id), ['a', 'b'], 'and the same from the other node');
    t.end();
});
...
t.test('a self-parenting record terminates at itself', t => {
    ...
    t.same(index.chain('a').map(node => node.id), ['a'], 'the record is seen once, not repeated');
...
t.test('a three-node cycle terminates without repeating a node', t => {
    ...
    t.same(index.chain('a').map(node => node.id), ['c', 'b', 'a'], 'every node is visited once');
    t.same(index.chain('b').map(node => node.id), ['a', 'c', 'b'], 'and the same from another entry point');
    t.end();
});
...
t.test('a record with no reference group still lands, grouped as untraced', t => {
    ...
    t.same(index.trace('untraced').map(node => node.id), ['loose']);
    t.same(index.chain('loose').map(node => node.id), ['loose'], 'it is its own root');
...
t.test('records sharing a timestamp are ordered by id, so a trace is stable', t => {
    ...
    t.same(index.trace('tr-1').map(node => node.id), ['a', 'b']);
    t.end();
});
...
t.test('a parent that never arrived leaves the record as its own root', t => {
    const index = new LineageIndex();
    index.add(event({id: 'orphan', time: 1, refs: {record: 'orphan', trace: 'tr-1', parent: 'missing'}}));
    t.same(index.chain('orphan').map(node => node.id), ['orphan']);
...
t.test('the trace array handed out is a copy, not the index itself', t => {
    ...
    handedOut.length = 0;
    t.same(index.trace('tr-1').map(node => node.id), ['n1', 'n2', 'n3'], 'the index survived the caller');
    t.end();
});
...
t.test('the nodes handed out are copies, not the index itself', t => {
    ...
    const added = index.add(event({id: 'n3', time: 30, refs: {record: 'n3', trace: 'tr-1', parent: 'n2'}}));
    added.id = 'injected';
    t.same(index.trace('tr-1').map(node => node.id), ['n1', 'n2', 'n3'], 'the listing survived the caller');
    t.same(index.chain('n2').map(node => node.id), ['n1', 'n2'], 'the walk survived the caller');
...
t.test('a trace that is empty or not a string is untraced, not a trace of its own', t => {
    ...
    t.same(index.trace('').map(node => node.id), [], 'nothing is grouped under the empty key');
    t.same(index.trace('untraced').map(node => node.id), ['empty', 'odd']);
    t.end();
});
...
t.test('a redelivered id is one listing entry, carrying the newest delivery', t => {
    ...
    t.same(index.trace('tr-1').map(node => node.id), ['r1']);
    ...
    t.same(index.chain('r1').map(node => node.id), ['r1'], 'the walk still sees one node');
    t.end();
});
...
t.test('a re-add under another trace moves the record; it is not listed under both', t => {
    ...
    t.same(index.trace('tr-a').map(node => node.id), [], 'the trace it left no longer claims it');
    t.same(index.trace('tr-b').map(node => node.id), ['m1'], 'the trace it moved to does');
...
t.test('a trace keeps its most recently indexed records and counts the least recent it dropped', t => {
    ...
    t.same(index.trace('tr-1').map(node => node.id), ['r2', 'r3'], 'the least recently indexed record was dropped, the newest kept');
    ...
    t.equal(index.rootOf('r3'), 'r2', 'so rootOf names a record closer to the leaf than the true origin');
    t.end();
});

t.test('a backdated arrival is kept: the record cap drops by arrival, not by the emitter clock', t => {
    const index = new LineageIndex(4, 2);
    ...
    t.equal(index.rootOf('r3'), 'r2', 'and the backdated record keeps its chain through the survivor');
    t.end();
});
