...
name: blong-ui
description: >
    Implement, extend, or debug the blong-ui React/TypeScript component library and portal
...
`@feasibleone/blong-ui` (`core/blong-ui/`) is a schema-driven React UI framework and portal shell.
...
## How blong-ui wires into the platform
...
```ts
import {widgetRegistry} from '@feasibleone/blong-ui';
...
`blongEvents` is a singleton typed event bus (exported from `@feasibleone/blong-ui`). It uses the
