...
When the backend is an HTTP service with an OpenAPI specification, the simulation uses the
`orchestrator.openapi` from `@feasibleone/blong-openapi`. The sim layer provides handler
implementations that respond to requests, and the orchestrator serves them via a local HTTP server.
...
When the backend is a TCP device (e.g., an HSM using a binary protocol like Payshield), the simulation
uses a second `adapter.tcp` instance with `listen: true`. This creates a TCP server that the regular
adapter connects to.
...
4. Both adapt use the same codec for protocol compatibility
