...
### ⚠️ `uidNotNull` PKs are NOT auto-generated by the generic `add`

The generic knex `exec` (`core/blong-gogo/src/adapter/server/knex.ts`) auto-generates PK values in
its `add` case **only** for columns that carry a `uuid` / `ulid` default marker (`type.uuid()` /
`type.ulid()` are submitted as the literal placeholder `'uuid'`/`'ulid'` and replaced server-side).
`type.uidNotNull()` columns have **no default marker**, so the generic `add` leaves the PK empty and
the insert fails — this has historically been a source of "PK is still empty" bugs.

If you make a `uidNotNull` PK a **foreign key to `core.resource.resourceId`** (a resource-backed
entity, like the access realm's `user`/`role`/`capability`/`action` tables) you have two options:

1. **Declare the table resource-backed** (`ISchemaTable.resource: true`, plus `edges` for
...
> **Wire format — responses:** all `binary(16)` columns round-trip through the RPC layer as
> **24-char base64** (`binaryToStr`), NOT as the 26-char Crockford ULID string or 36-char UUID.
> Assert on `length === 24` (or just that the generated value replaced the `'ulid'`/`'uuid'`
> placeholder), never 26/36.
>
> **Wire format — requests:** on the **input** side `strToBinary` accepts all four encodings, so a
> caller may submit a real **26-char Crockford ULID**, a **36-char dashed UUID**, 32-char hex, or
> 24-char base64 for any `binary(16)` column (get/find where, add/edit inserts, remove keys). The
> gateway does not restrict these (`type.ulid()`/`type.uuid()` have no format; `format: 'uid'` is
> not enforced by `Value.Check`), so no format-specific validation is needed.
